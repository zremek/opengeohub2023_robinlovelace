...
country_centroids = world_centroids |>
  st_filter(poland, .predicate = st_is_within_distance, dist = 2e5) # 2e5 == 2 * 10^5
countries = world |>
...
Let's create an animated map:

```{r}
#| eval: false
# ?tmap_animation
m_faceted = tm_shape(gpx_mutated[pois_buffer, ]) +
  tm_dots("minute", size = 0.5, legend.show = FALSE) +
  tm_facets("minute", free.coords = FALSE, ncol = 1, nrow = 1) +
  tm_scale_bar()
tmap_animation(m_faceted, delay = 10, filename = "gpx.gif")
```
...
::: {#fig-terra-plot-rgb-clamp layout-ncol="2"}
```{r}
#| layout-ncol: 2
# summary(r_stretch)
plotRGB(r_stretch)
...
```{r}
library(rsgeo)
countries_rs  = as_rsgeom(sf::st_geometry(countries_projected))
countries_rs
bench::mark(check = FALSE,
